...
namespace Api.Controllers
{
    ...
    {        
        ...
        public ConfigController(IMapper mapper,IConfigService configService , IUnitService unitService,ISimulationService simulationService) : base(mapper)
        {
        ...
        {
            var config = _configService.GetById(configId);
            _configService.DeleteAllConfigAccounts(configId); 
            if (config.AccountId != CustomUser.Id)
                return Unauthorized(new ResponseModel { Message = "Access Error!" });           
