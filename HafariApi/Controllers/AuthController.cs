...
namespace Api.Controllers
{
    ...
    public class AuthController : BaseController
    {
        ...
        public async Task<IActionResult> VerifyAndLogin([FromBody] LoginModel model)
        {
            // verify login
            var loginRes = await Login(new OTPLoginModel() {Username = model.Username, OrganizationUrl = model.OrganizationUrl});
            
            // login with password
            return await LoginWithPassword(model);
        }
